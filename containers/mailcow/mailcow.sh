...
installMailcow()
{
    ...
    if [[ "$mailcow" == *[iI]* ]]; then
		...
		local result=$(copyFile $install_containers_dir$app_name/docker-compose.$app_name.yml $containers_dir$app_name/docker-compose.$app_name.yml | sudo -u $easydockeruser tee -a "$logs_dir/$docker_log_file" 2>&1)
