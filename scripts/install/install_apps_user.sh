...
installJitsiMeet()
{
    ...
    if [[ "$jitsimeet" == *[iI]* ]]; then
		...
		result=$(rm -rf $install_path$app_name/$latest_tag.zip)
		checkSuccess "Deleting zip file to prevent conflicts"
		result=$(touch $latest_tag.txt && echo 'Installed "$latest_tag" on "$backupDate"!' > $latest_tag.txt)
		...
		result=$(mv $install_path$app_name/docker-jitsi-meet-$latest_tag/* $install_path$app_name)
		checkSuccess "Moving all files from zip file to install directory"
		result=$(rm -rf $install_path$app_name/$latest_tag.zip && rm -rf $install_path$app_name/$latest_tag/)
...
installActual()
{
    ...
    if [[ "$actual" == *[iI]* ]]; then
		...
		result=$(cd $install_path$app_name && ./gen-passwords.sh)
		checkSuccess "Running Actual gen-passwords.sh script"
		...
		if [ -f "$ssl_dir$ssl_key" ]; then
			...
			result=$(sudo cp $ssl_dir/$domain_full.crt $install_path$app_name/actual-data/cert.pem >> $logs_dir/$docker_log_file 2>&1)
			checkSuccess "Copying cert.pem to actual-data folder"

			result=$(sudo cp $ssl_dir/$domain_full.key $install_path$app_name/actual-data/key.pem >> $logs_dir/$docker_log_file 2>&1)
...
installMattermost()
{
    ...
    if [[ "$mattermost" == *[iI]* ]]; then
        ...
        result=$(sudo mkdir -p $install_path$app_name/volumes/app/mattermost/{config,data,logs,plugins,client/plugins,bleve-indexes})
