    ...
    if [[ "$CFG_REQUIREMENT_MANAGER" == "true" ]]; then
        if [[ "$OS" == [123] ]]; then
            if ! userExists "$CFG_DOCKER_MANAGER_USER"; then
                ...
                local result=$(sudo -u $easydockeruser useradd -m -s /bin/false "$CFG_DOCKER_MANAGER_USER")
                checkSuccess "Adding user via useradd command"
                
                local result=$(echo "$CFG_DOCKER_MANAGER_USER:$CFG_DOCKER_MANAGER_PASS" | sudo -u $easydockeruser chpasswd)
                checkSuccess "Setting up login password"

                local result=$(sudo -u $easydockeruser -u "$CFG_DOCKER_MANAGER_USER" mkdir -p /home/$CFG_DOCKER_MANAGER_USER/.ssh/)
                checkSuccess "Creating /home/$CFG_DOCKER_MANAGER_USER/.ssh folder"

                local result=$(sudo -u $easydockeruser -u "$CFG_DOCKER_MANAGER_USER" ssh-keygen -t ed25519 -b 4096 -f "/home/$CFG_DOCKER_MANAGER_USER/.ssh/ssh_key_${CFG_INSTALL_NAME}_${CFG_DOCKER_MANAGER_USER}" -N "passphrase")
                ...
                if [ -f "$config_file" ]; then
                    ...
                    local result=$(createTouch "$config_file")
...
local result=$(sudo -u $easydockeruser bash -c "cat >> /etc/ssh/sshd_config <<EOL
...
EOL")
                ...
                # Reload SSH Service
                local result=$(sudo -u $easydockeruser service ssh reload)
...
uninstallDockerManagerUser()
{
	...
	if [[ "$toolsremovedockermanageruser" == [yY] ]]; then
        if [[ "$OS" == [123] ]]; then
            ...
            # Restart SSH Service
            local result=$(sudo -u $easydockeruser service ssh restart)
