...
databaseInstallApp() {
    ...
    app_exists=$(sqlite3 "$base_dir/$db_file" "SELECT COUNT(*) FROM apps WHERE name = '$app_name';")

    if [ "$app_exists" -eq 0 ]; then
        isNotice "App does not exist in the database, setting up now."
        result=$(sqlite3 "$base_dir/$db_file" "INSERT INTO apps (name, status, install_date) VALUES ('$app_name', 1, date('now'));")
        checkSuccess "Adding $app_name to the apps database."
        echo ""
    else
        isNotice "App already exists in the database, updating now."
        result=$(sqlite3 "$base_dir/$db_file" "UPDATE apps SET status = 1, uninstall_date = NULL WHERE name = '$app_name';")
...
databaseUninstallApp() 
{
    ...
    # Check if the app exists in the database
    results=$(sqlite3 "$base_dir/$db_file" "SELECT name FROM apps WHERE name = '$app_name'")
    ...
    if [ -z "$results" ]; then
        ...
        if ! sqlite3 "$base_dir/$db_file" "UPDATE apps SET status = 0, uninstall_date = date('now') WHERE name = '$app_name';"; then
...
databaseAppScan() 
{
    ...
    folder_names=$(find "$install_path" -mindepth 1 -maxdepth 1 -type d -exec basename {} \;)
    ...
    existing_folders=$(sqlite3 "$base_dir/$db_file" "SELECT name, status FROM apps;")
        ...
        existing_folder_names+=("$folder_name")
            ...
            if [ -d "$install_path/$folder_name" ]; then
                if (( status != 1 )); then 
                    ...
                    result=$(sqlite3 "$base_dir/$db_file" "UPDATE apps SET status = 1, uninstall_date = NULL WHERE name = '$folder_name';")
                ...
                if (( status != 0 )); then
                    ...
                    result=$(sqlite3 "$base_dir/$db_file" "UPDATE apps SET status = 0, uninstall_date = date('now') WHERE name = '$folder_name';")
    ...
    for folder_name in $folder_names; do
        if [[ -n "$folder_name" ]]; then
            ...
            if ! [[ "${existing_folder_names[@]}" =~ "${folder_name}" ]]; then
                ...
                result=$(sqlite3 "$base_dir/$db_file" "INSERT INTO apps (name, status, install_date) VALUES ('$folder_name', 1, date('now'));")
...
databaseListAllApps()
{
	if [[ "$toollistallapps" == [yY] ]]; then
        ...
        output=$(sqlite3 -header -column $base_dir/$db_file "SELECT * FROM apps;")
...
databaseListInstalledApps() 
{
    ...
    results=$(sqlite3 "$base_dir/$db_file" "SELECT name, install_date FROM apps WHERE status = 1;")
...
databaseCycleThroughListApps()
{
    ...
    if [[ "$backupsingle" == [yY] ]] || [[ "$backupfull" == [yY] ]] || [[ "$migratesingle" == [yY] ]] || [[ "$migratefull" == [yY] ]]; then
        ...
        while IFS= read -r name; do
            app_names+=("$name")
        done < <(sqlite3 "$base_dir/$db_file" "SELECT name FROM apps WHERE status = 1;")
...
databaseDisplayTables() 
{
    if [[ "$toollistalltables" == [yY] ]]; then
        ...
        tables_list=$(sqlite3 "$base_dir/$db_file" ".tables")
        ...
        if [[ "$table_name" == "x" ]]; then
            echo "Exiting."
        return
        elif sqlite3 "$base_dir/$db_file" ".tables" | grep -q "\b$table_name\b"; then
            ...
            echo "##########################################"
            echo ""
            sqlite3 -column -header "$base_dir/$db_file" "SELECT * FROM $table_name;"
            echo ""
            read -p "Press Enter to continue..."
        else
...
databaseEmptyTable() 
{
  if [[ "$toolemptytable" == [yY] ]]; then
    ...
    if [[ "$table_name" == "x" ]]; then
      ...
      sqlite3 "$base_dir/$db_file" "DELETE FROM \"$table_name\";"
...
checkIfUpdateShouldRun() {
    ...
    latest_timestamp=$(sqlite3 "$base_dir/$db_file" "SELECT datetime(date || ' ' || time) FROM \"$table_name\" ORDER BY date DESC, time DESC LIMIT 1;")
    ...
    if [[ -n "$latest_timestamp" ]]; then
        ...
        if ((time_difference >= threshold)); then
            ...
            sqlite3 "$base_dir/$db_file" "UPDATE \"$table_name\" SET date='$current_date', time='$current_time' WHERE ROWID=1;"
        ...
        sqlite3 "$base_dir/$db_file" "INSERT INTO \"$table_name\" (date, time) VALUES ('$current_date', '$current_time');"
        return 0  # Return true (0)
    fi
}
...
databaseSSHKeysInsert()
{
    ...
    if [ "$key_in_db" -eq 0 ]; then
        ...
        result=$(sqlite3 "$base_dir/$db_file" "UPDATE $table_name SET name = '$key_file', date = '$current_date', time = '$current_time' WHERE name = '$key_file';")
...
databaseCronJobsInsert()
{
    ...
    if [ "$key_in_db" != "" ]; then
        if [ "$key_in_db" -eq 0 ]; then
            ...
            result=$(sqlite3 "$base_dir/$db_file" "UPDATE $table_name SET name = '$app_name', date = '$current_date', time = '$current_time' WHERE name = '$app_name';")
