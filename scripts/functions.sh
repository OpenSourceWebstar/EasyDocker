...
dockerDownUpDefault()
{
    ...
    if [[ "$OS" == "1" ]]; then
        result=$(su - $CFG_DOCKER_INSTALL_USER -c "cd $install_path$app_name && docker-compose down")
        checkSuccess "Shutting down container for $app_name"
        result=$(su - $CFG_DOCKER_INSTALL_USER -c "cd $install_path$app_name && docker-compose up -d")
        checkSuccess "Starting up container for $app_name"
    else
        result=$(su - $CFG_DOCKER_INSTALL_USER -c "cd $install_path$app_name && sudo docker-compose down")
        checkSuccess "Shutting down container for $app_name"
        result=$(su - $CFG_DOCKER_INSTALL_USER -c "cd $install_path$app_name && sudo docker-compose up -d")
        checkSuccess "Starting up container for $app_name"
    fi
}
...
dockerUpDownAdditionalYML()
{
    ...
    if [[ "$OS" == "1" ]]; then
        result=$(su - $CFG_DOCKER_INSTALL_USER -c "cd $install_path$app_name && docker-compose -f docker-compose.yml -f docker-compose.$app_name.yml down")
        checkSuccess "Shutting down container for $app_name (Using additional yml file)"
        result=$(su - $CFG_DOCKER_INSTALL_USER -c "cd $install_path$app_name && docker-compose -f docker-compose.yml -f docker-compose.$app_name.yml up -d")
        checkSuccess "Starting up container for $app_name (Using additional yml file)"
    else
        result=$(su - $CFG_DOCKER_INSTALL_USER -c "cd $install_path$app_name && sudo docker-compose -f docker-compose.yml -f docker-compose.$app_name.yml down")
        checkSuccess "Shutting down container for $app_name (Using additional yml file)"
        result=$(su - $CFG_DOCKER_INSTALL_USER -c "cd $install_path$app_name && sudo docker-compose -f docker-compose.yml -f docker-compose.$app_name.yml up -d")
        checkSuccess "Starting up container for $app_name (Using additional yml file)"
    fi
}
...
dockerStopAllApps()
{
    ...
    if [[ $CFG_REQUIREMENT_DOCKER_ROOTLESS == "true" ]]; then
        result=$(su - $CFG_DOCKER_INSTALL_USER -c "docker stop $(docker ps -a -q)")
...
dockerStartAllApps()
{
    ...
    if [[ $CFG_REQUIREMENT_DOCKER_ROOTLESS == "true" ]]; then
        result=$(su - $CFG_DOCKER_INSTALL_USER -c "docker restart $(docker ps -a -q)")
...
dockerAppDown()
{
    ...
    if [[ $CFG_REQUIREMENT_DOCKER_ROOTLESS == "true" ]]; then
        result=$(su - $CFG_DOCKER_INSTALL_USER -c "docker ps -a --format '{{.Names}}' | grep "$app_name" | xargs docker stop")
...
dockerAppUp()
{
    ...
    if [[ $CFG_REQUIREMENT_DOCKER_ROOTLESS == "true" ]]; then
        result=$(su - $CFG_DOCKER_INSTALL_USER -c "docker ps -a --format '{{.Names}}' | grep "$app_name" | xargs docker restart")
