...
dockerStopAllApps()
{
    ...
    if [[ $CFG_REQUIREMENT_DOCKER_ROOTLESS == "true" ]]; then
        result=$(runuser -l $CFG_DOCKER_INSTALL_USER -c "docker stop $(docker ps -a -q)")
...
dockerStartAllApps()
{
    ...
    if [[ $CFG_REQUIREMENT_DOCKER_ROOTLESS == "true" ]]; then
        result=$(runuser -l $CFG_DOCKER_INSTALL_USER -c "docker restart $(docker ps -a -q)")
...
dockerAppDown()
{
    ...
    if [[ $CFG_REQUIREMENT_DOCKER_ROOTLESS == "true" ]]; then
        result=$(runuser -l $CFG_DOCKER_INSTALL_USER -c "docker ps -a --format '{{.Names}}' | grep "$app_name" | xargs docker stop")
...
dockerAppUp()
{
    ...
    if [[ $CFG_REQUIREMENT_DOCKER_ROOTLESS == "true" ]]; then
        result=$(runuser -l $CFG_DOCKER_INSTALL_USER -c "docker ps -a --format '{{.Names}}' | grep "$app_name" | xargs docker restart")
