...
gitFolderResetAndBackup()
{
    ...
    local result=$(sudo rsync -av --include='*/' --include='*.config' --exclude='*' "$install_dir" "$backup_install_dir/$backupFolder/containers")
...
gitUntrackFiles()
{
    ...
    local config_files=($(sudo find "$containers_dir" -type f -name "*.config"))
